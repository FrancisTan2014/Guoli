  ...
  <section>
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="conditions" @submit.native.prevent="load">
        ...
        <el-form-item>
          <el-input v-model="conditions.RunSection.value" placeholder="运行区段"></el-input>
        </el-form-item>
        <el-form-item>
          <el-date-picker v-model="conditions.AnalysisStart.value" type="daterange" placeholder="分析日期" align="right">
          </el-date-picker>
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="data" highlight-current-row v-loading="isLoading" id="table">
      ...
      <el-table-column prop="TrainCode" label="分析车次"></el-table-column>
      <el-table-column prop="RunSection" label="运行区段"></el-table-column>
      <el-table-column prop="RunDate" label="运行日期" :formatter="RunDateFormatter"></el-table-column>
...
export default {
  data() {
    return {
      ...
      table: 'ViewInstructorAnalysis',
      order: 'AnalysisStart',
      desc: true,
      conditions: {
        Name: { value: undefined, type: 'like' },
        TrainCode: { value: undefined, type: 'like' },
        RunSection: { value: undefined, type: 'like' },
        AnalysisStart: { value: undefined, type: 'between' },
  ...
  methods: {
    ...
    RunDateFormatter: function (row) { return moment(row.RunDate).format('YYYY-MM-DD'); },
    ...
    detailPreview: function (index, row) {
      this.$router.push({ path: '/analysis-prev?id=' + row.Id, params: { data: row } });
    },
  },
