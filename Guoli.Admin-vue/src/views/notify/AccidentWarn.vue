  ...
  <section>
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="conditions" @submit.native.prevent="load">
        ...
        <el-form-item>
          ...
          <el-button type="primary" v-on:click="showAddForm()" icon="plus">添加预警</el-button>
        </el-form-item>
      </el-form>
    </el-col>
    ...
    <el-dialog :title="editFormModel.Id > 0 ? '修改' : '添加'" :visible="dialogVisible" :before-close="reset">
      ...
      <el-form ref="editForm" :model="editFormModel" :rules="editFormRules" label-width="120px">
        ...
        <el-form-item label="预警类型：" prop="AnnounceType">
...
export default {
  data() {
    return {
      ...
      conditions: {
        ...
        BusinessType: { value: 2, type: "equal" }
      },
  ...
  methods: {
    ...
    AnnounceTypeFormatter: function(row) {
      return row.AnnounceType === 1 ? "文件" : "文本";
    },
    ContentFormatter: function(row) {
      return row.AnnounceType === 1 ? row.FileName : row.Content;
    },
    PubTimeFormatter: function(row) {
      return moment(row.PubTime).format("YYYY-MM-DD HH:mm:ss");
    },
    ...
    handleEdit: function() {
      this.$refs.editForm.validate(valid => {
        if (valid) {
          ...
          let successMsg = m.Id > 0 ? "预警修改成功(:=" : "预警发布成功(:=";
          let failureMsg = m.Id > 0 ? "预警修改失败，请稍后重试(:=" : "预警发布失败，请稍后重试(:=";

          server
            .post(
              "/Announce/Save",
              { json: JSON.stringify(this.editFormModel) },
              this
            )
            .then(res => {
    ...
    handleDelete: function(row) {
      this.$confirm("您确定要删除此预警吗？")
        .then(() => {
          server.post("/Announce/Delete", { id: row.Id }, this).then(res => {
            if (res.code === 100) {
