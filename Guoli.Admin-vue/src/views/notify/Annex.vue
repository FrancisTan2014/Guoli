...
<template>
  <section>
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="conditions" @submit.native.prevent="load">
        ...
        <el-form-item>
          ...
          <el-button type="primary" v-on:click="showAddForm()" icon="plus">添加附件4</el-button>
        </el-form-item>
      </el-form>
    </el-col>
    ...
    <el-table :data="data" highlight-current-row v-loading="isLoading" id="table">
      ...
      <el-table-column prop="Title" label="附件4标题"></el-table-column>
      <el-table-column prop="AnnounceType" label="附件4类型" :formatter="AnnounceTypeFormatter"></el-table-column>
      <el-table-column prop="Content" label="附件4内容" :formatter="ContentFormatter"></el-table-column>
    ...
    <el-dialog :title="editFormModel.Id > 0 ? '修改' : '添加'" :visible="dialogVisible" :before-close="reset">
      ...
      <el-form ref="editForm" :model="editFormModel" :rules="editFormRules" label-width="120px">
        ...
        <el-form-item label="附件4类型：" prop="AnnounceType">
        ...
        <el-form-item label="附件4内容：" v-if="editFormModel.AnnounceType === 2" prop="Content">
  ...
  data() {
    return {
      ...
      conditions: {
        ...
        BusinessType: { value: 3, type: 'equal' }
      },
      ...
      editFormRules: {
        Title: [
          { required: true, message: '请输入附件4标题', trigger: 'blur' },
          { max: 30, message: '标题不能超过30个字', trigger: 'blur' }
        ],
        Content: [{ required: true, message: '请输入附件4内容', trigger: 'blur' }]
      }
    };
  },
    ...
    handleEdit: function () {
      this.$refs.editForm.validate(valid => {
        if (valid) {
          ...
          let successMsg = m.Id > 0 ? '附件4修改成功(:=' : '附件4发布成功(:=';
          let failureMsg = m.Id > 0 ? '附件4修改失败，请稍后重试(:=' : '附件4发布失败，请稍后重试(:=';
